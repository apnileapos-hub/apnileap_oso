              ...
              <div key={proj.id} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px' }}>
                ...
                <div style={{ marginBottom: '18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <h4 style={{ fontSize: '13px', color: 'var(--text-primary)', margin: 0 }}>📋 Project Epics & Board Reflection</h4>
                    ...
                    {user?.role !== 'College-SPOC' && (
                      addingEpicProjId === proj.id ? (
                        <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', padding: '14px', borderRadius: '6px', position: 'absolute', right: '40px', zIndex: 10, width: '280px', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            <input
                              ...
                              style={{ padding: '6px 10px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '4px', color: 'var(--text-primary)', fontSize: '12px' }}
                              required
                            />
                            <textarea
                              ...
                              style={{ padding: '6px 10px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '4px', color: 'var(--text-primary)', fontSize: '11px', height: '40px', resize: 'vertical' }}
                            />
                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                              <button
                                type="button"
                                onClick={() => setAddingEpicProjId(null)}
                                style={{ padding: '4px 10px', background: 'none', border: '1px solid var(--border)', color: 'var(--text-secondary)', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}
                              >
                                Cancel
                              </button>
                              <button
                        ...
                        <button
                          ...
                          style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: '#58a6ff', padding: '4px 10px', fontSize: '11px', borderRadius: '4px', cursor: 'pointer', fontWeight: '600' }}
                        >
                          ➕ Create Epic on Jira
                        </button>
                      )
                  ...
                  {!proj.epics || proj.epics.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
                      {proj.epics && proj.epics.map(epic => {
                        return (
                          <div key={epic.id} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', padding: '12px', borderRadius: '6px', position: 'relative' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                              ...
                              <span style={{ fontSize: '9px', fontWeight: 'bold', padding: '2px 6px', borderRadius: '10px', background: epic.status === 'Done' ? 'rgba(56,139,253,0.15)' : 'rgba(110,118,129,0.15)', color: epic.status === 'Done' ? '#58a6ff' : 'var(--text-secondary)' }}>
                                {epic.status}
                              </span>
                            </div>
