...
export default function ExecutiveHubView({ user }) {
  ...
  return (
    <div className="executive-hub-view" style={{ color: 'var(--text-primary)' }}>
      {/* Title Header */}
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '600', color: 'var(--text-primary)', margin: '0 0 6px 0' }}>🌐 Executive HUB</h1>
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>High-level overview of portfolio-wide allocations, campus spoke health, and external B2B funding metrics.</p>
      </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Loading portfolio overview...</div>
        </div>
      ) : (
        <div>
          {/* Portfolio KPI Row */}
          <div className="metrics-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
            <div className="metric-card" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '500', marginBottom: '8px' }}>Total Portfolio Funding</div>
              <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#3fb950' }}>${totalFunding.toLocaleString()}</div>
              <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '6px' }}>Total B2B client investments</div>
            </div>
            <div className="metric-card" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '500', marginBottom: '8px' }}>Active Spokes</div>
              <div style={{ fontSize: '28px', fontWeight: 'bold', color: 'var(--text-primary)' }}>{activeSpokesCount} / {spokes.length}</div>
              <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '6px' }}>Campus hubs with active projects</div>
            </div>
            <div className="metric-card" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '500', marginBottom: '8px' }}>Epic Completion Rate</div>
              <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#58a6ff' }}>{completionRate}%</div>
              <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '6px' }}>{completedEpicsCount} of {totalEpicsCount} Epics completed</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px', flexWrap: 'wrap' }}>
            ...
            <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)', margin: '0 0 16px 0' }}>💰 Funding Allocation by Spoke</h3>
                ...
                {spokeFunding.map(sf => {
                  ...
                  return (
                    <div key={sf.id}>
                      ...
                      <div style={{ background: 'var(--bg-primary)', height: '8px', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--bg-card)' }}>
            ...
            <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)', margin: '0 0 16px 0' }}>📊 Project Status Distribution</h3>
                ...
                {[
                  ...
                  return (
                    <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ fontSize: '13px', color: 'var(--text-primary)', flex: 1 }}>{s.label}</span>
                      <span style={{ fontSize: '13px', fontWeight: 'bold', color: 'var(--text-primary)', width: '30px', textAlign: 'right' }}>{s.count}</span>
                      <span style={{ fontSize: '11px', color: 'var(--text-secondary)', width: '40px', textAlign: 'right' }}>{pct}%</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)', margin: '0 0 16px 0' }}>📋 Active Projects Directory</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', textAlign: 'left' }}>
                  <th style={{ padding: '8px', color: 'var(--text-secondary)', fontWeight: '500' }}>Project</th>
                  <th style={{ padding: '8px', color: 'var(--text-secondary)', fontWeight: '500' }}>Partner</th>
                  <th style={{ padding: '8px', color: 'var(--text-secondary)', fontWeight: '500' }}>Allocated Spoke</th>
                  <th style={{ padding: '8px', color: 'var(--text-secondary)', fontWeight: '500' }}>Jira Sync Status</th>
                  <th style={{ padding: '8px', color: 'var(--text-secondary)', fontWeight: '500' }}>Progress</th>
                </tr>
              </thead>
              <tbody>
                {projects.map(p => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--bg-card)' }}>
                      <td style={{ padding: '12px 8px', fontWeight: '600', color: 'var(--text-primary)' }}>{p.title}</td>
                      <td style={{ padding: '12px 8px', color: 'var(--text-secondary)' }}>{p.company}</td>
                        ...
                        {p.spokeId ? (
                          ...
                          user?.role === 'Super-admin' || user?.role === 'Admin' ? (
                            <select
                              ...
                              style={{ padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '4px', color: '#ff7b72', fontSize: '11px', cursor: 'pointer' }}
                              defaultValue=""
                            >
                      ...
                      <td style={{ padding: '12px 8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <div style={{ width: '80px', background: 'var(--bg-primary)', height: '6px', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{ background: '#58a6ff', width: `${pct}%`, height: '100%' }} />
                          </div>
                          <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{pct}% ({done}/{total})</span>
                        </div>
                      </td>
                    </tr>
                  );
