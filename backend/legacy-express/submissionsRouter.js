  ...
  try {
    ...
    try {
      ...
      if (mockCheck.rows.length > 0) {
        ...
        // 2. Handle live GitLab issue transition
        console.log(`[REACTIVE AGENT] Resolving live GitLab task ${taskId} for automated status transition...`);
        const { transitionGitLabIssue, createGitLabIssueNote } = require('./gitlabService');
        
        const taskRes = await db.query('SELECT * FROM mock_tasks WHERE id = $1 OR key = $1', [taskId]);
        if (taskRes.rows.length > 0) {
          const task = taskRes.rows[0];
          const fields = typeof task.fields === 'string' ? JSON.parse(task.fields) : task.fields;
          const epicKey = fields.parent?.key;

          // Find the project containing this epic or matching the campus spoke
          const projRes = await db.query(
            `SELECT p.id, r.repo_name 
             FROM projects p
             LEFT JOIN repositories r ON r.project_id = p.id
             WHERE p.epics::text LIKE $1 OR p.spoke_id = (
               SELECT CASE 
                 WHEN $2 = '3' THEN 'kle-spoke'
                 WHEN $2 = '101' THEN 'coep-spoke'
                 WHEN $2 = '102' THEN 'mmcoep-spoke'
                 WHEN $2 = '103' THEN 'rit-spoke'
                 ELSE ''
               END
             )
             LIMIT 1`,
            [`%${epicKey}%`, task.board_id]
          );
          if (projRes.rows.length > 0) {
            const proj = projRes.rows[0];
            const gitlabProjectId = proj.repo_name;
            const issueIid = parseInt(task.key.split('-').pop());
            if (!isNaN(issueIid)) {
              await transitionGitLabIssue(gitlabProjectId, issueIid, targetStatusName);
              console.log(`[REACTIVE AGENT] Successfully transitioned live GitLab issue ${issueIid} to '${targetStatusName}'!`);
              if (status === "Re-work Requested") {
                await createGitLabIssueNote(gitlabProjectId, issueIid, `⚠️ [RE-WORK REQUESTED BY COORDINATOR]:\n${feedback}`);
                console.log(`[REACTIVE AGENT] Successfully appended re-work feedback comment to GitLab issue ${issueIid}.`);
              }
            }
          }
        }
