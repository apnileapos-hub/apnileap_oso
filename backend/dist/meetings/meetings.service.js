...
let MeetingsService = class MeetingsService {
    ...
    async createMeeting(body, actor = 'Scheduler') {
        if (!body.title || !body.campusId || !body.date || !body.time) {
            throw new common_1.BadRequestException('Missing required title, campusId, date, or time');
        }
    ...
    async getMeetingMessages(meetingId) {
        return this.prisma.meetingMessage.findMany({
            where: { meetingId },
            orderBy: { timestamp: 'asc' },
        });
    }
    async postMeetingMessage(meetingId, body, actor) {
        if (!body.text) {
    ...
    async sendPrepReminder(id, actor) {
        const meeting = await this.prisma.meeting.findUnique({
            where: { id },
        });
        if (!meeting) {
            throw new common_1.NotFoundException('Sync meeting not found');
        }
